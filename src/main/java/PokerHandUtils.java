...
public class PokerHandUtils {
    ...
    public static HandRankings evaluate(PokerHand hand) {
    ...
    private static List<Map.Entry<Character, Integer>> createCombination(Map<Character, Integer> rankCounts, Integer value) {
        //  System.out.println(rankCounts);
        return rankCounts.entrySet().stream()
                .filter(entry -> Objects.equals(entry.getValue(), value)) // Фильтруем карты, входящие в пару
                .map(entry -> Map.entry(entry.getKey(), CardPositions.cardPositions.get(entry.getKey()))) // Конвертируем в Map.Entry с весами
                .collect(Collectors.toCollection(ArrayList::new));
    }


    private static List<Map.Entry<Character, Integer>> createKickers(Map<Character, Integer> rankCounts, Integer value){
        return rankCounts.entrySet().stream()
                .filter(entry -> Objects.equals(entry.getValue(), value)) // Карты, которые не входят в пару
                .map(entry -> Map.entry(entry.getKey(), CardPositions.cardPositions.get(entry.getKey())))
                .sorted((a, b) -> Integer.compare(b.getValue(), a.getValue())) // Сортируем кикеры по убыванию
                .toList();
    }
    ...
    public static boolean isSet(PokerHand pokerHand) {
        Map<Character, Integer> rankCounts = pokerHand.getRankCounts();
    ...
    public static boolean isStraight(PokerHand pokerHand) {
        // System.out.println("start analyse: " + pokerHand.toString());
        List<Integer> positions = Arrays.stream(pokerHand.getCards())
                .map(card -> CardPositions.cardPositions.get(pokerHand.getRank(card)))
                .sorted().toList();
        ...
        if (regularStraight || wheelStraight) {
            ...
            if (wheelStraight) {
                combination.stream()
                        ...
                        .findFirst()
                        .ifPresent(entry -> {
                            combination.replaceAll(e -> e.getKey() == 'A' ? new AbstractMap.SimpleEntry<>(e.getKey(), 1) : e);
                        });
            }
            pokerHand.setCombination(combination);
        }
        // System.out.println("finish analyse: " + pokerHand.toString());
